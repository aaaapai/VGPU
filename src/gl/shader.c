...
bool can_run_essl3(int esversion, const char *glsl) {
	...
	return true;
    } else if (strncmp(glsl, "#version 100", 11) == 0) {
	return true;
...
void gl4es_glShaderSource(GLuint shader, GLsizei count, const GLchar * const *string, const GLint *length) {
	...
	int glsl_version = getGLSLVersion(glshader->source);
        ...
        if(glsl_version < 140 || globals4es.esversion < 300) {
            ...
            glshader->converted = GLSLtoGLSLES(glshader->source, glshader->type, globals4es.esversion);
            glshader->converted = strdup(glshader->converted!=NULL?ConvertShaderConditionally(glshader->converted):ConvertShaderConditionally(glshader));
